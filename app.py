...
#	name = StringField('What is your name?', validators=[()])
	number = StringField(u'考号', validators=[DataRequired()])
...
class SearchForm(FlaskForm):
	number = IntegerField(u'考号', validators=[DataRequired(message=u'请输入数字')])
...
class EditForm(FlaskForm):
	username = StringField(u'姓名', validators=[DataRequired()])
	number = IntegerField(u'考号', validators=[DataRequired(message=u'请输入数字')])
	password = StringField(u'密码', validators=[DataRequired(), Length(1,64),
									Regexp('^[a-zA-Z0-9_.]*$', 0,
											u'密码由字母、数字和_.组成')]),
...
def edit_user(id):
	...
	if form.validate_on_submit():
		...
		User.filter(username=form.username.data).update(username=form.username.data)
		User.filter(username=form.username.data).update(number=form.number.data)
		User.filter(username=form.username.data).update(role=Role.query.get(form.role.data))
